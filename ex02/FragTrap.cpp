...
FragTrap::FragTrap(): 	ClapTrap(), _vertuhaDmg(45), _runAwayAttackDmg(0),
	...
	std::srand(time(NULL));
	setDieMsg("This FR4G-TP " + this->getName() + " dead...");
	setType("FR4G-TP");
...
int FragTrap::vaulthunter_dot_exe(const std::string &target) {
	if (getEnergyPoints() && getHp()) {
		...
		std::cout << "FR4G-TP " << getName() << " prepares to attack and spends 25 energy, now he has "
				  << getEnergyPoints() << std::endl;
		return ((this->*superAttacks[rand() % 5])(target));
	} else {
		if (!getEnergyPoints())
			std::cout << "FR4G-TP " << getName() << " have not energy!\n";
		else
